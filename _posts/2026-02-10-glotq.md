...
The handler then parses the body again with its **own** parser — and this is where the desync happens.

---

## Root Cause — Parsing Desync

If we send `Content-Type: application/json` to `/yaml`:

- **Middleware** → parses with JSON parser → sees `Command: yq` ✅ (passes allowlist)
- **Handler** → parses with YAML parser → sees `command: man` 🚨 (runs man instead)
...
Whatever you put as `<browser>` gets executed as a command — so we pass `/readflag`, which is a SUID root binary that reads `/flag.txt`.
...
- This is a **Parsing Desync** attack — the same request body is parsed differently by two different parsers at validation and execution time.
