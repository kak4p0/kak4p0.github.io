---
title: "[Web] glotq"
description: Writing about the "glotq" of LA CTF 2026.
...
SUID root 바이너리 `/readflag`가 `/flag.txt`를 읽어 출력한다.   
핵심 취약점은 /yaml 엔드포인트에서 “검증(미들웨어)”과 “실행(핸들러)”이 서로 다른 파서를 사용한다는 점이다.   
미들웨어는 `Content-Type`이 `application/json`이면 json.Unmarshal로 Request{Command,Args}를 채워 검증하지만,   
핸들러는 /yaml이므로 yaml.Unmarshal로 다시 파싱해 실행한다.   
Go JSON 디코더의 필드 매칭은 대소문자에 느슨하고 동일 필드가 여러 번 등장하면 보통 마지막 값(last wins)이 남는다.   
