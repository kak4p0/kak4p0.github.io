...
- **Name:** megacorp
- **Category:** Web
- **Difficulty:** ★★★☆☆ (416 points, solving 66 out of 887 teams)

---

## TL;DR

Login as `alice:password123` → Find RSA public key at `/pubkey` → Forge admin JWT using **RS256-to-HS256 algorithm confusion** → Exploit **SSTI** in the profile bio field → Achieve **RCE** as root → Read the flag from the source code.
...
1. **JWT Algorithm Confusion** — Trick the server into accepting a forged token  
2. **SSTI (Server-Side Template Injection)** — Inject code through the bio field  
3. **RCE (Remote Code Execution)** — Run commands on the server to find the flag  
...
**Result:** `302 Redirect` → Login works! We get a JWT cookie.
...
- Alice's role is `"user"` — we need `"admin"` or `"sysadmin"`
- The signature is 256 bytes → 2048-bit RSA key
...
### 2) Root Cause — Algorithm Confusion
...
1. **The server accepts both RS256 and HS256** — It doesn't enforce a single algorithm
2. **The public key is accessible** — Found at `/pubkey`
3. **The same key object is used for both algorithms** — The server passes the RSA public key to the HMAC verification function when the token says "HS256"
...
# ========================================
# Step 4: SSTI → RCE → Read source code
# ========================================
...
alice:password123 (weak creds)
  ...
  JWT token (RS256, role: user)
       │
       ▼
  /pubkey → RSA public key leaked
       │
       ▼
  Algorithm Confusion (RS256 → HS256)
  Sign with public key → role: admin
       │
       ▼
  ...
  RCE as root (subprocess.check_output)
       │
       ▼
  Read source code → EH4X{14mk1nd4high}
```
